...
class Advertisement(models.Model):
    ...
    def updated_date(self):
        if self.update_at.date() == timezone.now().date():
            created_time = self.update_up.time().strftime("$H:%M:%S")
            return format_html(
                '<span style="color: green; fount-weight: bold;">Сегодня в {} </span>', created_time
            )
        return self.update_at.strftime("%d.%m.%Y в %H:%M:%S")

    def __str__(self):
        return f"Advertisement(id={self.id}, title={self.title}, prise={self.prise})"
